           ...
           // scriptFilter = "study3_n80";
            ...
            Script.find()
                ...
                .exec(function(err, script_feed) {
                    ...
                    let post_picturess = user.getPostInPeriod(time_limit, time_diff);
                    post_picturess.sort(function(a, b) {
                        return b.relativeTime - a.relativeTime;
                    });

                    //Array of recent user-made posts, within the last 5 minutes. They will be appended to the top of the final feed.
                    let new_post_picturess = [];

                    //While there are regular posts or user-made posts to add to the final feed
                    while (script_feed.length || post_picturess.length) {
                        // If there are no more script_feed posts
                        // Or If post_pictures[0] post is more recent than script_feed[0] post
                        if (typeof script_feed[0] === 'undefined' ||
                            (!(typeof post_picturess[0] === 'undefined') && (script_feed[0].time < post_picturess[0].relativeTime))) {
                            //Look at the post in post_picturess[0] now. 
                            post_picturess[0].comments.sort(function(a, b) {
                                return a.relativeTime - b.relativeTime;
                            });
                            if ((Date.now() - post_picturess[0].absTime) < 600000) {
                                //if post was made less than 10 minutes ago, it should be spliced into the TOP
                                new_post_picturess.push(post_picturess[0]);
                                post_picturess.splice(0, 1);
                            } else {
                                //proceed normally
                                finalfeed.push(post_picturess[0]);
                                post_picturess.splice(0, 1);
                            }
                        } else {
                    ...
                    finalfeed = new_post_picturess.concat(finalfeed);
